...
public class AutonomousStrafeToSideLift extends CommandGroup {
	...
	public AutonomousStrafeToSideLift(Direction direction) {
		...
		addSequential(new DriveForDistance(Direction.FORWARD, 4.3, 0.2));

		// rotate 60 degrees to the opposite angle
		if (direction == Direction.LEFT) {
			addSequential(new Rotate(Direction.RIGHT, 0.87, 0.3));
		} else if (direction == Direction.RIGHT) {
			addSequential(new Rotate(Direction.LEFT, 0.87, 0.3));
		}

		//addSequential(new DriveForDistance(Direction.FORWARD, 1, 0.2));
		// for front, 78.5in/6.6ft. For side, 46in
				addSequential(new DriveForDistance(Direction.FORWARD, 1.5, 0.2));
				...
				addSequential(new GearHandlerLower(1));
				// goes past the line for the 5 points(rotate, drive front for
				// horizontal)
				addSequential(new Rotate(direction, 0.87, 0.3));
